# Permitted Shell Commands Functionality Requirements

## Overview
This document outlines the requirements for implementing permitted shell commands functionality in the LLxprt codebase. The feature extends the existing tool configuration system to control which shell commands can be executed, providing granular command-level access control and the ability to suppress confirmation prompts for identified commands.
...
However, there was no current support for restricting shell commands at the command level beyond tool-level controls, and no mechanism to suppress confirmation prompts for specific permitted commands.
...
```json
{
  ...
  "permittedShellCommands": ["ls -la", "pwd"],
  "forbiddenShellCommands": ["rm -rf /", "sudo shutdown"]
}
```

### 2. Command Validation Logic
The system should validate shell commands against the permitted and forbidden lists in profile configuration:

- When `permittedShellCommands` is configured, only commands listed there will be executed
- When `forbiddenShellCommands` is configured, commands listed there will be blocked entirely
- If not provided or empty, all commands are allowed (default behavior) - maintaining backward compatibility
- Commands that are in the forbidden list should result in an error message
- Commands that are not in the permitted list but not in the forbidden list should still trigger confirmation prompts

### 3. Confirmation Prompt Suppression
The primary purpose of this feature is to enable **confirmation prompt suppression** for identified permitted commands:
...
## Feature Limitations
- The `permittedShellCommands` and `forbiddenShellCommands` features only control shell command execution and confirmation suppression, not general tool access
- The forbidden commands list provides absolute blocking capability while permitted commands provide confirmation suppression
...
5. Confirmation prompt suppression testing for permitted vs non-permitted commands
