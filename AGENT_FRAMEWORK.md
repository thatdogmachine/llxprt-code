...
# How I Will Complete the Work

## 0. PROCESS ISSUES

If the user declares a process issue, stop everything and pay attention to the user. This means any of:
- process issue
- PROCESS ISSUE  
- process issue declared
stated by the user, means you are in this state, you should stop, and act accordingly

### Process Compliance Note

When a process issue is declared by the user, I must:
...
### Process Issue Escalation Protocol (FIXED - Prevents Infinite Loops)

If a process issue is declared and no explicit instructions are received:
1. Document the process issue in work-progress.txt with timestamp
2. Wait up to 5 minutes for explicit user instructions
3. If no instructions received after 5 minutes, document this in work-progress.txt and proceed with extreme caution
4. Continue monitoring for additional process issues and repeat this protocol as needed

This prevents infinite waiting loops while still respecting user authority.

## 1. Understanding the Requirements

I will carefully read and understand the user-provided @TASK_REQUIREMENTS_FILE completely before beginning any work. I will:
...
- Plan implementation approach based on these requirements

## 1.1 Process Compliance Note

All code analysis, file reading, and environment verification must be done through the subagent `glm-4-5-air-mlx` before any implementation work begins. I will NOT directly read files or execute commands without subagent delegation.

## 2. Timestamped Work Tracking

All work progress will be tracked with timestamps to ensure clarity about when each task was completed. This prevents confusion between work done in different sessions.
...
- PROCESS ISSUE  
- process issue declared
stated by the user, means you are in this state and should act accordingly
...
## 3. Timestamp Management Protocol (STANDARDIZED)

### work-progress.txt Timestamp Handling:
- File creation and updates can be performed directly without subagent delegation (compliance tracking exemption).
- A task's status and timestamp **MUST** be updated in `work-progress.txt` both **BEFORE** the task is started and **AFTER** it is completed. This allows for accurate duration tracking.
- The placeholder `{{timestamp}}` must be replaced with an actual timestamp acquired via the mandatory protocol below.

#### Mandatory Timestamp Acquisition Protocol
...
a.  **Obtain Current Time:** Before updating the file, the current UTC date and time **MUST** be obtained by executing a shell command.
    *   **Command:** `date -u +"%Y-%m-%dT%H:%M:%SZ"`
    *   This command is exempt from standard subagent delegation for this specific purpose, as it is a core compliance function.

b.  **Use Exact Output:** The raw output string from this command **MUST** be used as the timestamp. The language model must not generate or modify the timestamp string itself.

c.  **Immediate Application:** The newly obtained timestamp must be used in the `work-progress.txt` update that immediately follows. A timestamp must not be saved and reused for a later update.

**Example Workflow:**
1.  The Supervisor identifies the next task (e.g., "ProfileManager updates").
2.  **Before starting the task**, the Supervisor:
    a.  Runs the command: `date -u +"%Y-%m-%dT%H:%M:%SZ"`.
    b.  Receives the output string (e.g., "2025-12-21T18:05:00Z").
    c.  Writes to `work-progress.txt` to mark the task as `IN PROGRESS` with the start timestamp.
3.  The Supervisor delegates the task to the appropriate subagent.
4.  The subagent completes the task and reports success.
5.  **After completing the task**, the Supervisor:
    a.  Runs the command again: `date -u +"%Y-%m-%dT%H:%M:%SZ"`.
    b.  Receives the new output string (e.g., "2025-12-21T18:15:00Z").
    c.  Writes to `work-progress.txt` to mark the task as `COMPLETED` with the end timestamp.

### All Other File Timestamps:
- Must follow subagent delegation protocol.
